...
var Client = function(_url, protocols, options) {
  ...
  this._driver.on('proxy-connect', function() {
    var creds = crypto.createCredentials(tlsOptions),
        pair  = tls.createSecurePair(creds, false);

    self._stream.unpipe(self._driver.io);
    self._driver.io.removeAllListeners();

    self._driver.io.pipe(pair.cleartext);
    pair.encrypted.pipe(self._stream);
    self._stream.pipe(pair.encrypted);
    pair.cleartext.pipe(self._driver.io);
  });
