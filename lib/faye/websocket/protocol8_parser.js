...
var instance = {
  ...
  parse: function(data) {
    for (var i = 0, n = data.length; i < n; i++) {
      switch (this._stage) {
        case 0:
          this._parseOpcode(data[i]);
          break;
          
        case 1:
          this._masked = (data[i] & this.MASK) === this.MASK;
          this._length = (data[i] & this.LENGTH);
        ...
        case 2:
          this._lengthBuffer.push(data[i]);
          if (this._lengthBuffer.length === this._lengthSize) {
        ...
        case 3:
          this._mask.push(data[i]);
          if (this._mask.length === 4) this._stage = 4;
          break;
          
        case 4:
          this._payload.push(data[i]);
          if (this._payload.length === this._length) this._emitFrame();
          break;
      }
  ...
  _emitFrame: function() {
    var payload = new Buffer(this._payload),
        opcode  = this._opcode;
    
   if (this._mask.length) {
       payload = this._unmask(this._payload, this._mask)
   }
