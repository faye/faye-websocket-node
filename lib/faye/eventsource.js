...
var EventSource = function(request, response, options) {
  ...
  this._pingLoop = setInterval(function() {
    try { this._response.socket.write(':\r\n\r\n') } catch (e) {}
  }, this._ping * 1000);
  
  ['close', 'end', 'error'].forEach(function(event) {
    self._response.socket.addListener(event, function() { self.close() });
  });
  ...
  try {
    response.socket.write(handshake, 'utf8');
  } catch (e) {}
};
...
var instance = {
  ...
  send: function(message, options) {
    ...
    try {
      this._response.socket.write(frame, 'utf8');
    } catch (e) {}
  },
