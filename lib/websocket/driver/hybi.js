...
var crypto     = require('crypto'),
    util       = require('util'),
    Base       = require('./base'),
    Extensions = require('./hybi/extensions'),
...
var Hybi = function(request, url, options) {
  ...
  var self = this;
  this._extensions.on('error', function(error) {
    self._fail('extension_error', error.message);
  });
...
var instance = {
  ...
  frame: function(data, type, code) {
    ...
    frame.opcode  = this.OPCODES[type || (isText ? 'text' : 'binary')];
    ...
    this._sendFrame(frame, true);
    return true;
  },

  _sendFrame: function(frame, runExtensions) {
    if (runExtensions && this.MESSAGE_OPCODES.indexOf(frame.opcode) >= 0) {
      ...
      return this._extensions.processOutgoingMessage(message, function(message) {
        for (var i = 0, n = message.frames.length; i < n; i++)
          this._sendFrame(message.frames[i]);
      }, this);
    }
  ...
  _parseOpcode: function(data) {
    ...
    if (!this._extensions.validFrame(frame))
  ...
  _emitFrame: function(buffer) {
    ...
    if (opcode === this.OPCODES.text) {
      this._message = new Message('text');
      this._message.pushFrame(frame);
    }

    if (opcode === this.OPCODES.binary) {
      this._message = new Message('binary');
      this._message.pushFrame(frame);
    }

    if (frame.final && this.MESSAGE_OPCODES.indexOf(opcode) >= 0)
      return this._emitMessage(this._message, true);
  ...
  _emitMessage: function(message, runExtensions) {
    if (runExtensions)
      return this._extensions.processIncomingMessage(message, this._emitMessage, this);
