...
var Client = function(_url, options) {
  ...
  this.readyState = this._proxy ? -2 : -1;
...
var instance = {
  start: function() {
    ...
    if (this.readyState === -2) {
      this._write(this._proxyConnectRequest());
      this.readyState = -1;
  ...
  parse: function(data) {
    ...
    if (this.readyState === -1)
      this._validateProxyResponse();
    else
      this._validateHandshake();

    if (this.readyState === 1) this.parse(this._http.body);
  },

  _proxyConnectRequest: function() {
    var proxy = url.parse(this._proxy),
        uri   = url.parse(this.url),
        port  = uri.port || PORTS[uri.protocol],
        auth  = proxy.auth && new Buffer(proxy.auth, 'utf8').toString('base64');

    var headers = [ 'CONNECT ' + uri.hostname + ':' + port + ' HTTP/1.1',
                    'Host: ' + uri.host,
  ...
  _handshakeRequest: function() {
    ...
    var headers = [ 'GET ' + pathname + ' HTTP/1.1',
                    'Host: ' + uri.host,
                    'Upgrade: websocket',
                    'Connection: Upgrade',
    ...
    headers = headers.concat(this.__headers.toString(), '');

    return new Buffer(headers.join('\r\n'), 'utf8');
  },

  _validateProxyResponse: function() {
    if (this._http.statusCode === 200) {
      this._http = new HttpParser('response');
      this.emit('proxy-connect');
      this.start();
    } else {
      var message = "Can't establish a connection to the server at " + this.url;
      this.emit('error', new Error(message));
      this.readyState = 3;
      this.emit('close', new Base.CloseEvent(1006, ''));
    }
  },
